...
import { ACTION_KEYS, ACTION_PRESENTATION, clearFrameMotion, completeActionArrival, completedVisibleVelocity, createActionState, directionTo, migrateActionState, setAction, setBlockedAction } from "../src/action-state.js";

test("every authoritative action key has one exhaustive presentation entry", () => {
  const required = ["idle", "rest", "travel", "wander", "graze", "browse", "drink", "flee", "join-herd", "evaluate-prey", "stalk", "chase", "attack", "search", "listen", "track-scent", "guard", "defend", "blocked", "courtship", "reject", "scavenge", "nurse", "communicate"];
...
test("load/reset frame cleanup removes interpolation and trail-driving motion", () => {
  ...
  assert.deepEqual(animal, { visualMove: null, motionTarget: null, moveIntent: null, movementNoise: 0 });
});
