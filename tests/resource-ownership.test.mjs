...
test("entity-owned canvas maps are disposed with their material while shared semantic maps survive", () => {
  const ownedMap = resource(RESOURCE_OWNERSHIP.entity), ownedMaterial = resource(RESOURCE_OWNERSHIP.entity), sharedMap = resource(RESOURCE_OWNERSHIP.shared), sharedMaterial = resource(RESOURCE_OWNERSHIP.entity);
  ownedMaterial.map = ownedMap; sharedMaterial.map = sharedMap;
  const disposed = disposeOwnedTree({ traverse(fn) { fn({ material: [ownedMaterial, sharedMaterial] }); } });
  assert.equal(disposed, 3);
  assert.equal(ownedMap.disposed, 1);
  assert.equal(ownedMaterial.disposed, 1);
  assert.equal(sharedMaterial.disposed, 1);
  assert.equal(sharedMap.disposed, 0);
});
