...
export function runPredictiveCognition(animal, { tick = 0, mode, profile, modelBudget = null, disabledModels = [], automatic = mode == null } = {}) {
  ...
  if (budget >= 3 && targetEvidence.length) { const observed = strongestEvidence(targetEvidence), prior = state.activations.lastTargetObservation?.[observed.targetId], elapsed = Math.max(1, tick - Number(prior?.tick || tick - 1)), inferredVx = prior ? (observed.x - prior.x) / elapsed : 0, inferredVz = prior ? (observed.z - prior.z) / elapsed : 0, directVelocity = observed.velocityConfidence >= .18 && Number.isFinite(observed.vx) && Number.isFinite(observed.vz), vx = directVelocity ? observed.vx : inferredVx, vz = directVelocity ? observed.vz : inferredVz, motionConfidence = directVelocity ? observed.velocityConfidence : clamp(observed.motionConfidence || observed.confidence * .45), uncertainty = Number(observed.velocityUncertainty || 0); candidates.push(prediction(animal, tick, "motion.v1", "TREND_EXTRAPOLATION", "TARGET_POSITION", { position: { x: observed.x + vx * 4 * state.learned.motionScale, z: observed.z + vz * 4 * state.learned.motionScale }, radius: 1.5 + 4 * (1 - observed.confidence) + uncertainty * 4, velocity: { x: vx, z: vz }, velocityConfidence: motionConfidence, velocityUncertainty: uncertainty, temporalResolutionHz: observed.temporalResolutionHz }, observed.confidence * state.confidence.motion * (.55 + motionConfidence * .45), [observed], { referent: observed.targetId, horizon: 4 })); state.activations.lastTargetObservation = { ...(state.activations.lastTargetObservation || {}), [observed.targetId]: { x: observed.x, z: observed.z, tick } }; }
