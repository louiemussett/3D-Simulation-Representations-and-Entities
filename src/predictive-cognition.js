...
export function captureAnimalEvidence(animal, tick) {
  const contacts = (animal.sensoryBuffer || []).map((item, index) => Object.freeze({ evidenceId: item.evidenceId || `sense:${animal.id}:${tick}:${index}`, channel: item.channel || "unknown", originalChannel: item.originalChannel || item.channel || "unknown", provenance: item.provenance || (item.communicatedBy || item.signalKind ? "communication" : "perception"), type: item.type || item.kind || "unknown", targetId: item.targetId || null, communicatedBy: item.communicatedBy || null, signalKind: item.signalKind || null, x: Number.isFinite(item.x) ? item.x : null, z: Number.isFinite(item.z) ? item.z : null, vx: Number.isFinite(item.vx) ? item.vx : null, vz: Number.isFinite(item.vz) ? item.vz : null, motionConfidence: clamp(item.motionConfidence ?? 0), velocityConfidence: clamp(item.velocityConfidence ?? 0), velocityUncertainty: Number.isFinite(item.velocityUncertainty) ? item.velocityUncertainty : null, temporalResolutionHz: Number.isFinite(item.temporalResolution?.effectiveHz) ? item.temporalResolution.effectiveHz : null, distance: Number.isFinite(item.distance) ? item.distance : null, confidence: clamp(item.confidence ?? .5), age: 0 }));
...
export function runPredictiveCognition(animal, { tick = 0, mode, profile, modelBudget = null, disabledModels = [], automatic = mode == null } = {}) {
  ...
  if (budget >= 3 && targetEvidence.length) { const observed = strongestEvidence(targetEvidence), prior = state.activations.lastTargetObservation?.[observed.targetId], elapsed = Math.max(1, tick - Number(prior?.tick || tick - 1)), inferredVx = prior ? (observed.x - prior.x) / elapsed : null, inferredVz = prior ? (observed.z - prior.z) / elapsed : null, directVelocity = observed.velocityConfidence >= .18 && Number.isFinite(observed.vx) && Number.isFinite(observed.vz), inferredVelocity = Number.isFinite(inferredVx) && Number.isFinite(inferredVz), vx = directVelocity ? observed.vx : inferredVelocity ? inferredVx : null, vz = directVelocity ? observed.vz : inferredVelocity ? inferredVz : null, motionConfidence = directVelocity ? observed.velocityConfidence : inferredVelocity ? clamp(observed.motionConfidence || observed.confidence * .45) : clamp(observed.motionConfidence || observed.confidence * .25), velocityConfidence = directVelocity || inferredVelocity ? motionConfidence : 0, uncertainty = Number(observed.velocityUncertainty || 0), horizon = 4 * state.learned.motionScale; candidates.push(prediction(animal, tick, "motion.v1", "TREND_EXTRAPOLATION", "TARGET_POSITION", { position: { x: observed.x + (vx || 0) * horizon, z: observed.z + (vz || 0) * horizon }, radius: 1.5 + 4 * (1 - observed.confidence) + uncertainty * 4 + (inferredVelocity || directVelocity ? 0 : 2.5), velocity: vx == null ? null : { x: vx, z: vz }, motionState: observed.motionState || (vx == null ? "unknown" : "possibly-moving"), velocityConfidence, velocityUncertainty: uncertainty, temporalResolutionHz: observed.temporalResolutionHz }, observed.confidence * state.confidence.motion * (.55 + motionConfidence * .45), [observed], { referent: observed.targetId, horizon: 4 })); state.activations.lastTargetObservation = { ...(state.activations.lastTargetObservation || {}), [observed.targetId]: { x: observed.x, z: observed.z, tick } }; }
