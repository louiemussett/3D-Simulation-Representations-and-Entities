...
export function vegetationLod(distance, cameraDistance, quality = 1, { largeMapPerformanceMode = false } = {}) { const q = Math.max(.5, Math.min(1.5, Number(quality) || 1)), visibleRadius = Math.max(66, Math.min(210 * q, cameraDistance * .76 + 38)), normalFineRadius = Math.max(34, Math.min(86 * q, cameraDistance * .62)), normalMediumRadius = Math.max(normalFineRadius, Math.min(135 * q, cameraDistance * .92)), fineRadius = largeMapPerformanceMode ? normalFineRadius * .6 : normalFineRadius, mediumRadius = largeMapPerformanceMode ? Math.max(fineRadius, normalMediumRadius * .7) : normalMediumRadius; return { visible: distance <= visibleRadius, fine: distance <= fineRadius, medium: distance <= mediumRadius, visibleRadius, fineRadius, mediumRadius }; }
